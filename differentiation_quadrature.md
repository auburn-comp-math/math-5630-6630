...
Let $f(x)$ be a real-valued polynomial with a simple root $x^{\ast}$. Suppose the bisection method produces a sequence of approximation $\{x_k\}_{k=1}^n$ with a finite length $n$, show that 

- The Wynn's $\eps$ method cannot accelerate the convergence of the sequence to $x^{\ast}$. 
